...
- **Planning feedback.** Before an arm moves, the interface shows an animated search bar, elapsed solve time, and the current solver phase: **trying IK candidates**, **checking collision-safe route**, or **reducing verified route**. Select **Cancel solver** to stop before execution; once a route is found, the bar changes to exact control-step progress.
...
- **Step budget.** The **Step budget** slider sets the maximum number of control steps (1–500). Each scenario loads its recommended horizon, including 500 for Towel fold — its deterministic pin/lift/cross/place sequence plus the cloth solver's settle tail runs to about 477 steps; raise the budget to inspect a longer run or lower it to test bounded failure handling.
...
Scenario 07 now identifies its task-specific browser controller and renders a spring-cloth approximation in the 3-D viewport. The towel is a spring network built from its triangle mesh (stretch, shear and bending springs) advanced by compute-shader-style kernel passes after [three-simplecloth](https://github.com/bandinopla/three-simplecloth). It responds to gravity, table contact with Coulomb friction and fold-layer separation, and only attaches to a corner once the corresponding gripper reaches it; it is not morphed merely because a progress counter advances. The visible 120-frame rolling buffer reports its captured corners and whether the mesh has settled. Its physics parameters (spring stiffnesses and damping, gravity, sub-steps, friction, layer thickness, grasp and release radii) live on their own page, `cloth.html` (**Cloth settings** in the nav), with a live preview fold and a stability estimate; values persist in the browser and apply to an open lab tab immediately. It remains an intentionally limited browser approximation: it does not model general self-collision, material anisotropy, or real gripper contact. Training a learned policy would require recorded demonstrations, camera observations, a training runtime, and evaluation data that this static repository does not include.
